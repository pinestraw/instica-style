...
## 2.8 Hero gradient tokens (hardware hero cues)
...
Use these tokens to recreate the soft gradients from the provided hero captures. Keep gradient stops stored as tokens so marketing and product surfaces stay aligned.
...
## 2.9 Doc console tokens (split-shell docs)
...
These tokens enable a split-shell developer experience: pale content on the left, console-style status blocks on the right—matching the provided references.
